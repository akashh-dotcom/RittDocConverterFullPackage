...
<xsl:template match="footnote/para[1]|footnote/simpara[1]" priority="2">
  ...
  <xsl:variable name="label" select="ancestor::footnote/@label"	/>	
  <xsl:variable name="tableid" select="ancestor::table/@id"	/>

 <!--<xsl:if test="../preceding::footnote[ancestor::table[@id = $tableid] and @label = $label ]  and $label != '' ">
  <xsl:message>Looking at dupe lable <xsl:value-of select="$label" /> <xsl:value-of select="ancestor::footnote/@id" /></xsl:message></xsl:if> -->

 <xsl:choose>
		<xsl:when test="../preceding::footnote[ ancestor::table[@id = $tableid] and @label = $label ]  and $label != '' "><a id="{$name}" name="{$name}"></a></xsl:when>
		<xsl:otherwise>
  <p>
    <sup>
      <xsl:text>[</xsl:text>
...
<!-- foot note processing at the section level needs to consider all nodes of the parent -->
	<xsl:param name ="footnotes" select=".//footnote" />
  <!--<xsl:variable name="footnotes" select=".//footnote" />-->
  <xsl:variable name="table.footnotes"
                select=".//table//footnote" />
                <xsl:variable name="anc.root" select="ancestor::*[@id = $rootid]"	/>
...
<xsl:template match="footnote"  mode="process.footnote.mode">
 <!-- <xsl:if test="$rootid = '' or anc.root[1]/@id = $rootid"> -->
  <xsl:choose>
    <xsl:when test="local-name(*[1]) = 'para' or local-name(*[1]) = 'simpara'">
	<!-- ## 27/12/05 If statement addted to remove the footnote if within the table -->
      <xsl:if test="not(ancestor::table)">
		  <div class="{concat(name(.),'test') }">
			  <xsl:apply-templates/>
		  </div>
	  </xsl:if>      
    </xsl:when>
