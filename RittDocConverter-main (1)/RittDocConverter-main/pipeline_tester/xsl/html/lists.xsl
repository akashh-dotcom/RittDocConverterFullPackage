    ...
    <ul>
      <xsl:if test="$css.decoration != 0">
        <xsl:attribute name="class">
          ...
          <xsl:choose><xsl:when test="$mark = 'none'">bullets-none</xsl:when></xsl:choose>
					<xsl:choose><xsl:when test="$mark = '•'">bullets</xsl:when></xsl:choose>
					<xsl:choose><xsl:when test="$mark = '○'">circles</xsl:when></xsl:choose>
					<xsl:choose><xsl:when test="$mark = '–'">ndash</xsl:when></xsl:choose>
					<xsl:choose><xsl:when test="$mark = '▪'">squares</xsl:when></xsl:choose>
        </xsl:attribute>
      </xsl:if>
...
<!--  27-10-2005 To preserve original styling Unicodes are converted -->
  <xsl:variable name="cssmark">
    <xsl:choose>
      <xsl:when test="$usemark = 'opencircle' or $usemark = '&#9675;'">circle</xsl:when>
      <xsl:when test="$usemark = 'bullet' or $usemark = '&#8226;' or $usemark = '&#9679;'">disc</xsl:when>
      <xsl:when test="$usemark = 'box' or $usemark = '&#9632;'">square</xsl:when>
      <xsl:otherwise>
