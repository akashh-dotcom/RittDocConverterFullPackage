...
<xsl:template match="inlinemediaobject" mode="no.anchor.mode">
...
<xsl:template match="inlinemediaobject">
  <span class="{name(.)}">
    <!--<xsl:if test="@id">
      <a id="{@id}" name="{@id}"></a>
    </xsl:if>--><xsl:call-template name="select.mediaobject"/></span>
</xsl:template>
...
<xsl:template match="textobject">
	<xsl:apply-templates>
		<xsl:with-param name="content">
			<xsl:choose>
				<xsl:when test="self::textobject[parent::mediaobject[videoobject]]">
					<!--<xsl:comment> field: r2VideoTitle </xsl:comment>-->
					<xsl:element name="a">
						<xsl:attribute name="id"><xsl:value-of select="ancestor::mediaobject[1]/@id" /></xsl:attribute>
						<xsl:attribute name="class">video-link</xsl:attribute>
						<xsl:attribute name="href">#videoContainer</xsl:attribute>
						<xsl:attribute name="data-file">
							<xsl:value-of select="ancestor::mediaobject[1]/videoobject/videodata/@fileref" />
						</xsl:attribute>
						<xsl:attribute name="data-toggle">modal</xsl:attribute>
						<xsl:apply-templates select="para/node()" mode="no.anchor.mode" />
					</xsl:element>
					<!--<xsl:comment> field: </xsl:comment>-->
				</xsl:when>
				<xsl:when test="self::textobject[parent::mediaobject[audioobject]]">
					<xsl:element name="a">
						<xsl:attribute name="id">
							<xsl:value-of select="ancestor::mediaobject[1]/@id" />
						</xsl:attribute>
						<xsl:attribute name="class">video-link</xsl:attribute>
						<xsl:attribute name="href">#videoContainer</xsl:attribute>
						<xsl:attribute name="data-file">
							<xsl:value-of select="ancestor::mediaobject[1]/audioobject/audiodata/@fileref" />
						</xsl:attribute>
						<xsl:attribute name="data-toggle">modal</xsl:attribute>
