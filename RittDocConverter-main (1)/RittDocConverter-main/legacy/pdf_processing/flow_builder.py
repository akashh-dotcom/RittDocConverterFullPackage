    ...
    *,
    ...
    class XMLBuilder:
        ...
        def _emit_sidebar(self, block: TextBlock) -> None:
            ...
            # DTD Requirement: Sidebars must be inside sect1, not direct children of chapter
            # Check if current container is a chapter (level 1)
            parent = self._current_container()
            current_level = self.stack[-1][1] if self.stack else 1

            # If we're at chapter level (1), create a sect1 wrapper
            if current_level == 1:
                # Create a section to hold the sidebar
            ...
            # Validate content before creating sidebar
            text_content = block.text.strip() if block.text else ""
            if not text_content:
                # Skip empty sidebars (violates DTD requirement for 1+ children)
                return
