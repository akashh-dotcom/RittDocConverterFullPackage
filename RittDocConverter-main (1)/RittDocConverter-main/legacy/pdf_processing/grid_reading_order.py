...
Handles coordinate system differences between pdftohtml (origin at bottom-left,
Y increases upward) and PyMuPDF (origin at top-left, Y increases downward)
by converting between coordinate systems before comparing positions.
...
class TableDetector:
    ...
    def _find_table_for_caption(self, page: fitz.Page, caption_info: Dict, 
        ...
        """
        # Convert pdftohtml coordinates to PyMuPDF coordinates
        ...
        # Convert table bbox back to pdftohtml coordinates for text comparison
...
class GridPipeline:
    ...
    def _load_pages_with_tables(self, xml_path: Path, images_by_page: Dict[int, List[Dict]]) -> List[Dict]:
        ...
        for page_elem in root.findall('.//page'):
            ...
            # Extract fontspec elements from this page
    ...
    def _create_output_xml(self, pages: List[Dict]) -> None:
        ...
        for page_data in pages:
            ...
            # Write fontspec elements first (before any content)
            fontspec_count = 0
        ...
        # Verify fontspecs were preserved
        print("\n" + "="*70)
        ...
        if total_fontspecs > 0:
            ...
            print(f"WARNING: No fontspec elements found in output!")
