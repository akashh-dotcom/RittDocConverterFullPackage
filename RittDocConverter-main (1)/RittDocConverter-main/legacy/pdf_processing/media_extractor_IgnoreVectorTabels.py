...
- Vector drawings (diagrams, flowcharts)
- Raster images (photos, embedded images)
- Tables are automatically detected and skipped

Features:
- Overlap-based merging (2pt tolerance)
- Caption text matching
- Text boundary expansion
            ...
            if page_drawings:
                ...
                for region in figures_only:
                    ...
                    if width <= 0 or height <= 0:
                        print(f"         {media_name}: Invalid dimensions ({width:.1f}x{height:.1f}), skipped")
                        continue
                    
                    if width < 5 or height < 5:
                        print(f"         {media_name}: Too small ({width:.1f}x{height:.1f}), skipped")
                        vector_decorative_count += 1
                        continue
                    ...
                    if clip_rect.is_empty or clip_rect.width < 1 or clip_rect.height < 1:
                        print(f"         {media_name}: Invalid clip region, skipped")
                        continue
                    ...
                    try:
                        ...
                        if pix.width == 0 or pix.height == 0:
                            print(f"         {media_name}: Empty pixmap ({pix.width}x{pix.height}), skipped")
                            continue
                        ...
                        print(f"         {media_name}: Save failed ({str(e)}), skipped")
                        continue
...
def main():
    ...
    print("- Tables automatically detected & skipped")
