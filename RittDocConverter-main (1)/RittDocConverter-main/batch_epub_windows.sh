...
detect_python() {
    case "$OS_TYPE" in
        Windows)
            # Prefer Windows launcher, then python3, then python
            if command -v py &> /dev/null; then
                if py -3 -c "import sys; print(sys.version_info.major)" &> /dev/null; then
                    PYTHON_CMD="py -3"
    ...
    if ! PY_VERSION=$($PYTHON_CMD -c 'import sys; print(sys.version_info.major)' 2>&1); then
        echo -e "${RED}Error: $PYTHON_CMD failed to run Python. Output: $PY_VERSION${NC}"
        exit 1
    fi
...
# Ensure Python output handles Unicode on Windows terminals.
...
setup_venv() {
	...
	"./.venv/Scripts/python.exe" -m pip install -q --upgrade pip setuptools wheel 2>/dev/null \
