...
class BatchProcessor:
    ...
    def _run_pipeline(self, input_file: Path, output_dir: Path) -> bool:
        """
        Run the EPUB pipeline on a file.
        ...
        try:
            ...
            if ext in ['.epub', '.epub3']:
                pipeline_script = self.root_dir / "epub_pipeline.py"
            else:
                # PDF processing has been moved to legacy
                # If you need PDF processing, use a separate PDF pipeline project
                self.logger.error(f"Unsupported format: {ext}. Only EPUB files are supported.")
                return False
            ...
            cmd = [
                ...
                str(input_file),
                str(output_dir),
                "--no-interactive"  # Disable interactive prompts for batch mode
            ]
