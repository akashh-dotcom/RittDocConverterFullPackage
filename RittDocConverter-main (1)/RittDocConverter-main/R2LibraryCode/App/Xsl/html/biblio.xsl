...
<xsl:template match="authorgroup" mode="bibliography.mode">
  <span class="{name(.)}">
    ...
    <xsl:if test="not(parent::partinfo)">
    <xsl:value-of select="$biblioentry.item.separator"/>
    </xsl:if>
  </span>
</xsl:template>
...
<xsl:template match="degree" mode="bibliography.mode">
  <span class="{name(.)}">
    <xsl:apply-templates mode="bibliography.mode"/>
    <xsl:value-of select="$biblioentry.item.separator"/>
  </span>
...
<xsl:template match="bibliocoverage|bibliorelation|bibliosource"
              mode="bibliography.mode">
  <span class="{name(.)}">
    <xsl:apply-templates mode="bibliography.mode"/>
    <xsl:value-of select="$biblioentry.item.separator"/>
...
<!-- ## 11/07/06 linebreak inside bibliography -->
<xsl:template match="processing-instruction('lb')" mode="bibliomixed.mode"><br/></xsl:template>
...
<xsl:template match="editor" mode="bibliomixed.mode">
  <span class="{name(.)}">
    <xsl:apply-templates mode="bibliomixed.mode"/>
    <!-- ## 22/10/07 Added for space and punctuation after editor in bib -->
    <xsl:value-of select="$biblioentry.item.separator"/>   
  </span>
</xsl:template>
