...
<xsl:template name="inline.superscriptseq">
  ...
  </xsl:param><sup><xsl:if test="@dir">
      <xsl:attribute name="dir">
        <xsl:value-of select="@dir"/>
      </xsl:attribute>
    </xsl:if><xsl:copy-of select="$content"/></sup></xsl:template>
...
<xsl:template name="inline.emphasis">
  <!--<span>
    <xsl:choose>
...
<!--  **25-11-06** html\inline.xsl - To add space between city and state-->
	<xsl:if test="name(.)='state' and preceding-sibling::*[1][name(.)='city']"><![CDATA[ ]]></xsl:if>
  <xsl:call-template name="inline.charseq"/>
</xsl:template>
...
<xsl:template match="honorific|firstname|surname|lineage|othername|degree">
  <xsl:call-template name="inline.charseq"/>
</xsl:template>
