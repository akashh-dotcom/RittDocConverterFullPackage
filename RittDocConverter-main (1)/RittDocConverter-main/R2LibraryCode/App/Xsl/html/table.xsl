...
<xsl:template match="tgroup" name="tgroup">
  ...
  <table>
    ...
    <xsl:apply-templates select="thead|tgroup/thead"/>
    <xsl:apply-templates select="tfoot|tgroup/tfoot"/>
    <xsl:apply-templates select="tbody|tgroup/tbody"/>

    <xsl:if test=".//footnote and not(self::entrytbl)">
...
<!-- ## 09-10-06 updated for last row having footnote ##-->
      <xsl:when test="not(ancestor::row[1]/following-sibling::row)
                          and not(ancestor::tbody/preceding-sibling::tfoot)
                          and ancestor::table[1]//footnote
                          and @rowsep='1'">
        <xsl:value-of select="1"/>
      </xsl:when>
<!-- **09-12-06** for last row having rowsep 1 -->
       <xsl:when test="@rowsep='1' and not(ancestor::row/following-sibling::row)"><xsl:value-of select="1"/></xsl:when>
      <xsl:when test="not(ancestor-or-self::row[1]/following-sibling::row
                          or ancestor-or-self::thead/following-sibling::tbody
                          or ancestor-or-self::tbody/preceding-sibling::tfoot)">
        <xsl:value-of select="0"/>
