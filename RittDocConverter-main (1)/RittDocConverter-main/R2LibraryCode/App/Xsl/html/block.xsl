...
<!-- ##15/11/05 To show boxes below para "admon xsl to remove box in para match"## -->
...
<xsl:template match="formalpara/para">
  <xsl:apply-templates/>
 <!-- ##16/11/05 To show boxes in Formalpara."admon xsl to remove box in para match"## -->
     <xsl:apply-templates select="note|sidebar|important|warning|caution|tip" mode="anticipated" />
</xsl:template>
...
<xsl:template match="blockquote">
  ...
  <!-- ## 04/08/06 match for role="sm" in blockquote to reduce font size -->
	<!--<xsl:if test="@role='sm'"><xsl:attribute name="class"><xsl:value-of select="local-name(.)"/>sm</xsl:attribute></xsl:if>-->
    ...
    <!--<xsl:choose>
      <xsl:when test="attribution">-->
        <table border="0" width="100%"
               cellspacing="0" class="blockquote"
               summary="Block quote">
          <tr>
