...
class ValidationReportGenerator:
    ...
    # Elements that require ID attribute per RittDoc DTD
    ELEMENTS_REQUIRING_ID = {'anchor'}

    # Elements that should have IDs for proper cross-referencing
    ELEMENTS_RECOMMENDED_ID = {
        # Document-level elements
        'book', 'chapter', 'preface', 'appendix', 'glossary',
        'bibliography', 'index', 'colophon', 'dedication', 'acknowledgments',
        'part', 'article',
        # Section elements
        'sect1', 'sect2', 'sect3', 'sect4', 'sect5', 'simplesect',
        # Block elements commonly referenced
        'figure', 'table', 'example', 'equation', 'procedure',
        'footnote', 'sidebar',
        # Bibliography/glossary entries
        'bibliomixed', 'biblioentry', 'glossentry',
    }
    ...
    def _categorize_dtd_error(self, message: str) -> str:
        ...
        if 'no declaration' in message_lower or 'not declared' in message_lower:
            ...
            return 'DTD Validation Error'  # This will be auto-fixed by comprehensive_dtd_fixer
        elif 'duplicate' in message_lower and 'id' in message_lower:
            return 'DTD Validation Error'
        elif 'id' in message_lower and 'required' in message_lower:
    ...
    def validate_ids(self, xml_filename: str, xml_tree) -> int:
        """
        Validate IDs in an XML document for:
        1. Missing required IDs (per RittDoc DTD)
        2. IDs exceeding maximum length (25 chars)
        3. Duplicate IDs

        Args:
            xml_filename: Name of the XML file being validated
            xml_tree: lxml ElementTree or Element to validate

        Returns:
            Number of ID-related errors found
        """
        if not LXML_AVAILABLE or xml_tree is None:
            return 0

        error_count = 0
        seen_ids = {}  # Track IDs for duplicate detection
        ...
        for elem in root.iter():
            ...
            # Check 1: Missing required IDs
            if tag_name in self.ELEMENTS_REQUIRING_ID:
                if not elem_id:
                    self.errors.append(ValidationError(
                        xml_file=xml_filename,
                        line_number=line_num,
                        column_number=None,
                        error_type='Missing Required ID',
                        error_description=f'The <{tag_name}> element requires an "id" attribute but none was provided',
                        severity='Error'
                    ))
                    error_count += 1

            # Check 2: ID length exceeds maximum
            if elem_id and len(elem_id) > self.MAX_ID_LENGTH:
                self.errors.append(ValidationError(
                    xml_file=xml_filename,
                    line_number=line_num,
                    column_number=None,
                    error_type='ID Too Long',
                    error_description=f'ID "{elem_id}" on <{tag_name}> is {len(elem_id)} characters (max {self.MAX_ID_LENGTH})',
                    severity='Error'
                ))
                error_count += 1
            ...
            if elem_id:
                if elem_id in seen_ids:
                    prev_line = seen_ids[elem_id]
                    self.errors.append(ValidationError(
                        xml_file=xml_filename,
                        line_number=line_num,
                        column_number=None,
                        error_type='Duplicate ID',
                        error_description=f'ID "{elem_id}" is duplicated. First occurrence at line {prev_line or "unknown"}',
                        severity='Error'
                    ))
                    error_count += 1
                else:
                    seen_ids[elem_id] = line_num

            # Check 4: Recommended IDs (warning only)
            if tag_name in self.ELEMENTS_RECOMMENDED_ID:
                if not elem_id:
                    self.errors.append(ValidationError(
                        xml_file=xml_filename,
                        line_number=line_num,
                        column_number=None,
                        error_type='Missing Recommended ID',
                        error_description=f'The <{tag_name}> element should have an "id" attribute for proper cross-referencing',
                        severity='Warning'
                    ))
                    # Don't increment error_count for warnings

        return error_count

    def validate_ids_from_file(self, xml_path: Path) -> int:
        """
        Validate IDs in an XML file.
        ...
        if not LXML_AVAILABLE:
            print("Warning: lxml not available, skipping ID validation")
            return 0
        ...
        try:
            ...
            return self.validate_ids(xml_path.name, tree)
        except etree.XMLSyntaxError as e:
            self.add_xml_syntax_error(xml_path.name, e)
            return 1
        except Exception as e:
            self.add_general_error(
                xml_path.name,
                'ID Validation Error',
                f'Failed to validate IDs: {str(e)}',
                'Error'
            )
            return 1
    ...
    def _create_summary_sheet(self, wb: Workbook, book_title: str):
        ...
        row += 1
        if total_errors == 0:
            ws[f'A{row}'] = '[OK] PASSED - No errors found'
            ...
            ws[f'A{row}'] = f'[FAIL] FAILED - {total_errors} error(s) found'
    ...
    def _save_workbook_with_retry(self, wb: Workbook, output_path: Path, max_retries: int = 3):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Try to save
                wb.save(output_path)
                print(f"[OK] Validation report saved -> {output_path}")
                return  # Success!
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"  [WARN] File is locked (attempt {attempt + 1}/{max_retries}). "
                          f"Retrying in {wait_time} seconds...")
                    print(f"    (If you have the Excel file open, please close it)")
                    time.sleep(wait_time)
                else:
                    # Final attempt failed
                    print(f"\n[FAIL] ERROR: Cannot save validation report - file is locked")
                ...
                print(f"[FAIL] Error saving validation report: {e}")
                raise
    ...
    def print_summary(self):
        ...
        print(f"\n[FAIL] Found {len(self.errors)} validation error(s):")
        print("=" * 80)
...
# =============================================================================
# CROSS-FILE IDREF VALIDATION
# =============================================================================

def build_global_id_index(zip_path: Path, logger: Optional[logging.Logger] = None) -> Set[str]:
    """
    Build a set of all IDs across all XML files in a ZIP package.

    This allows filtering of false positive IDREF errors that reference
    valid IDs in other files (cross-file references).
    ...
    Returns:
        Set of all IDs found across all XML files
    """
    all_ids: Set[str] = set()
    ...
    # Regex pattern to extract id attributes from XML content
        ...
        for xml_path in xml_files:
            ...
            try:
                ...
                for elem in root.iter():
                    elem_id = elem.get('id')
                    if elem_id:
                        file_ids.add(elem_id)
                parsed_ok = True

            except Exception:
                # Second try: Recovery parser (handles malformed XML/entities)
                try:
                    recover_parser = etree.XMLParser(recover=True, remove_blank_text=False)
                    tree = etree.parse(str(xml_path), recover_parser)
                    root = tree.getroot()
            ...
            # Third try: Regex fallback for files with entity references
            if not parsed_ok or xml_path.name.lower() == 'book.xml':
                try:
                    content = xml_path.read_text(encoding='utf-8', errors='ignore')
                    regex_ids = set(id_pattern.findall(content))
                    if regex_ids:
                        # For Book.XML, prefer regex results as they capture all IDs
                        # even from entity-referenced content
                        if xml_path.name.lower() == 'book.xml':
                            file_ids.update(regex_ids)
                        elif not parsed_ok:
                            file_ids = regex_ids
                except Exception:
                    pass
    ...
    if logger:
        logger.debug(f"Global ID index built with {len(all_ids)} IDs")

    return all_ids


def filter_cross_file_idref_errors(
    errors: List[ValidationError],
    global_ids: Set[str]
) -> Tuple[List[ValidationError], List[ValidationError]]:
    """
    Filter IDREF errors to separate true errors from valid cross-file references.

    Cross-file references (e.g., <link linkend="ch0005"> in ap0001.xml) are valid
    but may appear as IDREF errors when validating each file in isolation.

    Args:
        errors: List of ValidationError objects from DTD validation
        global_ids: Set of all IDs across all files

    Returns:
        Tuple of (true_errors, filtered_cross_file_refs)
    """
    true_errors = []
    filtered_refs = []

    # Pattern to extract linkend value from error messages
    idref_pattern = re.compile(r"IDREF attribute linkend references an unknown ID \"([^\"]+)\"")
    linkend_pattern = re.compile(r"linkend[='\"]([^'\"]+)['\"]")
    unknown_id_pattern = re.compile(r"references unknown ID \"?([^\"'\s]+)\"?")

    for error in errors:
        # Check if this is an IDREF error
        if 'IDREF' in error.error_description or 'unknown ID' in error.error_description.lower():
            # Try to extract the referenced ID
            referenced_id = None
            ...
            if match:
                ...
                match = linkend_pattern.search(error.error_description)
                if match:
                    referenced_id = match.group(1)
                else:
            ...
            if referenced_id and referenced_id in global_ids:
                # This is a valid cross-file reference, not a true error
                error.severity = 'Info'
                error.error_type = 'Cross-File Reference'
                error.error_description = f"Valid cross-file reference to '{referenced_id}' (exists in another file)"
                filtered_refs.append(error)
            else:
                true_errors.append(error)
        else:
            true_errors.append(error)

    return true_errors, filtered_refs


def validate_cross_file_idrefs(
    zip_path: Path,
    errors: List[ValidationError]
) -> Tuple[List[ValidationError], int]:
    """
    Validate IDREF errors against global ID index and filter valid cross-file references.

    This is a convenience wrapper that combines build_global_id_index and
    filter_cross_file_idref_errors.
    ...
    Returns:
        Tuple of (filtered_errors, count_of_valid_cross_file_refs)
    """
    global_ids = build_global_id_index(zip_path)
    true_errors, filtered_refs = filter_cross_file_idref_errors(errors, global_ids)
    return true_errors, len(filtered_refs)
...
def generate_validation_report(
    ...
    if generator.has_errors():
        ...
        print(f"\n[OK] No validation errors found for {book_title}")
