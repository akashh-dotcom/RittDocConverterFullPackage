...
class ConversionMetadata:
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "total_images": self.num_vector_images + self.num_raster_images,
...
class ConversionTracker:
    ...
    def complete_conversion(self,
        ...
        # Push to MongoDB (if available)
        self._push_to_mongodb()
    ...
    def _push_to_mongodb(self) -> None:
        """Push current conversion data to MongoDB (if available)."""
        if not self.current_metadata:
            return

        if not MONGODB_AVAILABLE:
            logger.debug("MongoDB not available - skipping push")
            return

        try:
            client = get_mongodb_client()
            result = client.push_conversion(self.current_metadata.to_dict())

            if result:
                logger.info(f"Pushed conversion data to MongoDB: {self.current_metadata.filename}")
            else:
                logger.warning(f"Failed to push to MongoDB: {self.current_metadata.filename}")

        except Exception as e:
            # Don't fail the conversion if MongoDB push fails
            logger.warning(f"MongoDB push failed (non-fatal): {e}")
