        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for entity_name, filename in sorted(entities.items()):
                ...
                if chapter_errors:
                    ...
                    print(f"  {filename}: [OK] Valid")

            # Run ID validation on all chapters
            print("Validating IDs (required, length <=25, uniqueness)...")
            for entity_name, filename in sorted(entities.items()):
                chapter_path = base_dir / filename
                if chapter_path.exists():
                    try:
                        ...
                        tree = etree.parse(str(chapter_path), parser)
                        id_errors = report.validate_ids(filename, tree)
                        if id_errors > 0:
                            print(f"  {filename}: {id_errors} ID issue(s)")
                    except Exception as e:
                        print(f"  {filename}: ID validation error - {e}")

        # Generate Excel report if requested
        if output_report_path and report.has_errors():
            report.generate_excel_report(output_report_path, "RittDoc Package")
            print(f"\n[OK] Validation report saved: {output_report_path}")
...
def main():
    ...
    if report.has_errors():
        ...
        print(f"[FAIL] Found {error_count} validation error(s)")
        ...
        print("[OK] Package is DTD-compliant")
        sys.exit(0)
