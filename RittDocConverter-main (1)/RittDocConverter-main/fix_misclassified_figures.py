...
2. The figure contains no real image (empty mediaobject/imageobject)
...
def fix_misclassified_figures(xml_content: str) -> Tuple[str, int, List[Tuple[int, str]]]:
    ...
    def replace_func(match):
        ...
        # Check if the figure has a real image (imagedata with fileref)
        # Empty figures (no imagedata or empty imageobject) are candidates for conversion
        ...
        if not has_real_image and has_table_data:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for chapter_file in sorted(chapter_files):
            ...
            if fixes > 0:
                ...
                print(f"  [OK] {chapter_file.name}: Fixed {fixes} misclassified figure(s)")
                ...
                print(f"  [FAIL] {chapter_file.name}: {status}")
