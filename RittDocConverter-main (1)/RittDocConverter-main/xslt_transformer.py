...
# XML parser with entity resolution for structured.xml files that use external entities
ENTITY_RESOLVING_PARSER = etree.XMLParser(load_dtd=True, resolve_entities=True, no_network=True)
...
def apply_xslt_transform(
    ...
    if isinstance(xml_input, Path):
        ...
        xml_doc = etree.parse(str(xml_input), ENTITY_RESOLVING_PARSER)
