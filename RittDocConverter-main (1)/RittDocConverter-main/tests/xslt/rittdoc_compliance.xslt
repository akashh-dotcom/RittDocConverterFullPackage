...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="section">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$depth = 6">
                <!-- RittDoc DTD supports sect6 via ritthier2.mod -->
                <sect6>
                    <xsl:choose>
                        <xsl:when test="@id">
                            <xsl:apply-templates select="@*|node()"/>
            ...
            <xsl:otherwise>
                <!-- If nesting exceeds 6 levels, use sect6 with warning -->
                <xsl:comment>Warning: Section nesting exceeds 6 levels - converted to sect6</xsl:comment>
                <sect6>
                    <xsl:choose>
                        <xsl:when test="@id">
                            <xsl:apply-templates select="@*|node()"/>
    ...
    <!-- Template to generate section IDs based on chapter/sect1 conventions -->
    <xsl:template name="generate-section-id">
        <xsl:param name="node" select="."/>

        <!-- Chapter ID should already be present as ch####; fall back to ch0000 -->
        <xsl:variable name="chapter-id">
            <xsl:choose>
                <xsl:when test="$node/ancestor::chapter[1]/@id">
                    <xsl:value-of select="$node/ancestor::chapter[1]/@id"/>
                </xsl:when>
                <xsl:otherwise>ch0000</xsl:otherwise>
            </xsl:choose>
        </xsl:variable>

        <!-- Generate 0-based hierarchical IDs:
             sect1: ch####s0000, sect2: ch####s0000s0000, etc. -->
        <xsl:call-template name="generate-section-id-rec">
            <xsl:with-param name="node" select="$node"/>
            <xsl:with-param name="chapter-id" select="$chapter-id"/>
        </xsl:call-template>
    </xsl:template>

    <xsl:template name="generate-section-id-rec">
        <xsl:param name="node"/>
        <xsl:param name="chapter-id"/>

        <xsl:choose>
            <xsl:when test="$node/parent::section">
                <xsl:call-template name="generate-section-id-rec">
                    <xsl:with-param name="node" select="$node/parent::section"/>
                    <xsl:with-param name="chapter-id" select="$chapter-id"/>
                </xsl:call-template>
                <xsl:text>s</xsl:text>
                <xsl:value-of select="format-number(count($node/preceding-sibling::section), '0000')"/>
            </xsl:when>
            <xsl:otherwise>
    ...
    <!-- Ensure proper author structure - but ONLY when no affiliation -->
    <!-- DTD: author = ((personname | (firstname|surname|affiliation|...)+), ...) -->
    <!-- When affiliation exists, firstname/surname must NOT be wrapped in personname -->
    <xsl:template match="author[not(personname) and (firstname or surname) and not(affiliation)]">
        <author>
            <xsl:apply-templates select="@*"/>
            <personname>
    ...
    <xsl:template match="informalfigure">
        <figure>
            <xsl:apply-templates select="@*"/>
            <!-- Add empty title if missing (DTD allows empty title) -->
    ...
    <xsl:template match="figure[not(table)]">
        <figure>
            ...
            <!-- Ensure title exists (MANDATORY per DTD, but can be empty) -->
            <xsl:choose>
                <xsl:when test="title">
                    <xsl:apply-templates select="title"/>
                </xsl:when>
    ...
    <!-- ============================================================== -->
    <!-- IMAGE SIZING: Ensure all images scale properly in HTML output  -->
    <!-- ============================================================== -->

    <!-- Add width="100%" and scalefit="1" to all imagedata elements -->
    <!-- This ensures images don't overflow their containers when rendered -->
    <xsl:template match="imagedata">
        <imagedata>
            <!-- Copy all existing attributes -->
            <xsl:apply-templates select="@*"/>

            <!-- Add width="100%" if not already specified -->
            <xsl:if test="not(@width)">
                <xsl:attribute name="width">100%</xsl:attribute>
            </xsl:if>

            <!-- Add scalefit="1" if not already specified -->
            <xsl:if test="not(@scalefit)">
                <xsl:attribute name="scalefit">1</xsl:attribute>
            </xsl:if>

            <!-- Copy any child nodes (imagedata is typically EMPTY, but just in case) -->
            <xsl:apply-templates select="node()"/>
        </imagedata>
    </xsl:template>
    ...
    <!-- Handle existing sect1-sect6 elements that don't have IDs -->
    <!-- Auto-generate IDs using chapter/section numbering convention -->
    <xsl:template match="sect1[not(@id)]|sect2[not(@id)]|sect3[not(@id)]|sect4[not(@id)]|sect5[not(@id)]|sect6[not(@id)]">
    ...
    <xsl:template match="sect1[not(*[not(self::title) and not(self::subtitle) and not(self::titleabbrev) and not(self::sect1info)])]
                       ...
                       |sect6[not(*[not(self::title) and not(self::subtitle) and not(self::titleabbrev) and not(self::sect6info)])]">
        <xsl:element name="{local-name()}">
            <!-- Copy all attributes -->
            <xsl:apply-templates select="@*"/>

            <!-- Copy info, title, subtitle, titleabbrev if present -->
            <xsl:apply-templates select="*[self::sect1info or self::sect2info or self::sect3info or self::sect4info or self::sect5info or self::sect6info or self::title or self::subtitle or self::titleabbrev]"/>

            <!-- Add empty para to make section non-empty (empty para is valid per DTD) -->
            <para/>

            <!-- Copy any other remaining children -->
            <xsl:apply-templates select="*[not(self::sect1info) and not(self::sect2info) and not(self::sect3info) and not(self::sect4info) and not(self::sect5info) and not(self::sect6info) and not(self::title) and not(self::subtitle) and not(self::titleabbrev)]"/>
        </xsl:element>
    </xsl:template>
