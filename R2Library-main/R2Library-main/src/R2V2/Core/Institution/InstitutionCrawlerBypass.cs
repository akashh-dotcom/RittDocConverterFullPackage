...
{
    [Serializable]
    public class InstitutionCrawlerBypass : AuditableEntity, ISoftDeletable, IDebugInfo
    {
        ...
        {
            return new StringBuilder("InstitutionCrawlerBypass = [ ")
                .AppendFormat("Id: {0}", Id)
                .AppendFormat(", [{0}.{1}.{2}.{3}]", OctetA, OctetB, OctetC, OctetD)
                .AppendFormat(", [{0}]", UserAgent)
