...
{
    public class InitiatePromotionBatch : R2MessageBase
    {
        ...
        public PromoteRequest[] PromoteRequests { get; set; }

        public new string ToDebugString()
        {
            var sb = new StringBuilder("InitiatePromotionBatch = [");
            ...
            sb.Append(", PromoteRequests: [");
            if (PromoteRequests != null)
            {
                foreach (var promoteRequest in PromoteRequests)
                {
                    sb.AppendLine().AppendFormat(", {0}", promoteRequest.ToDebugString());
                }
            }
