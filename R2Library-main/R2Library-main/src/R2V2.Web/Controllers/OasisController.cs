...
{
    public class OasisController : BaseApiController
    {
        ...
        private readonly RapidOrderService _rapidOrderService = ServiceLocator.Current.GetInstance<RapidOrderService>();

        //Cannot use DI without updating Autofac across all projects. 

        [HttpGet]
        public IHttpActionResult GetData([FromUri] string key)
        {
            if (key != _collectionManagementSettings.OasisSecretKey)
            {
        ...
        public IHttpActionResult PostData([FromUri] string key)
        {
            ...
            var processedOrder = _rapidOrderService.ProcessOrder(RapidOrderType.OASIS, order, out var emailMessage,
                out var emailSubject);
            var emailProperties = new EmailPage
            {
                From = "admin@rittenhouse.com",
                To = _collectionManagementSettings.OasisRapidOrderRecipients,
