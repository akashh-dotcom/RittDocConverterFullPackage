...
<xsl:template match="address">
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
      <div class="{name(.)}">
      	<xsl:choose>
      	<!-- 27-11-06 Condition if address appears in sect1info or chapterinfo -->
      	<xsl:when test="ancestor::sect1info or ancestor::chapterinfo">
<!-- ## 01/12/05 choose added to remove 160;(spaces) within the auddress(in sectinfo) which is handled in "titlepage.ritt.xsl" template name="Author-name-formatting" while ori. is in otherwise Also for the affiliation within the address -->
	      <xsl:choose>
			<xsl:when test="node()[ancestor::author]">
				<xsl:if test="not(descendant::affiliation)">
					<xsl:copy-of select="$rtf"/>
				</xsl:if>
				<xsl:if test="descendant::affiliation">
        ...
        <xsl:otherwise>
<!-- **30-08-07** <p> removed to apply style -->
	<!-- <p> -->
<!-- ## 01/12/05 choose added to remove 160;(spaces) within the auddress(in sectinfo) which is handled in "titlepage.ritt.xsl" template name="Author-name-formatting" while ori. is in otherwise Also for the affiliation within the address -->
	      <xsl:choose>
			<xsl:when test="node()[ancestor::author]">
...
<!-- ## 30/12/05 to add AFFILIATION within the chapterinfo though address is not parent Find titlepage.ritt.xsl for the new match for the same. -->
<xsl:template match="affiliation">
  ...
  <xsl:variable name="rtf">
    <xsl:apply-templates/>
      ...
      <div class="{name(.)}">
        <p><xsl:choose>
			<xsl:when test="node()[ancestor::author]"><xsl:copy-of select="$rtf"/></xsl:when>
			<xsl:otherwise>
				<xsl:call-template name="make-verbatim">
					<xsl:with-param name="rtf" select="$rtf"/>
				</xsl:call-template>
