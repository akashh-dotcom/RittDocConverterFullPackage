 ...
 * @author vbhatia
 */
public class DrugSynonymRISIndexThread implements Runnable {

	//logger
	protected Category log = Category.getInstance(DrugSynonymRISIndexThread.class.getName());
	...
	public DrugSynonymRISIndexThread(Document doc, Node titleNode, String ruleId, String bookIsbn){
		this.doc = doc;
	...
	public void run() {
	    Iterator drugSynonymIt = Main.foundDrugSynList.keySet().iterator();
		while (drugSynonymIt.hasNext()) {
			boolean subTitleFound = false;
			boolean titleFound = false;
			Integer drugSynonymId = (Integer) drugSynonymIt.next();
			Integer drugId = (Integer) MetaData.getDrugSynonymXDrugMetaData().get(drugSynonymId);
			String drugSynonymName = (String) MetaData.getDrugSynonymMetaData().get(drugSynonymId);
			String drugName = (String) MetaData.getDrugMetaData().get(drugId);
			String termToSearch = drugSynonymName.toLowerCase();
			String termToTag = drugName;
			try{
				...
				String topicToTag = topicValue;
					...
					try {
							...
							if (topicIndex > -1) {//sect1 found topic
								...
								addDrugSynonymIndexDB(bookIsbn, drugSynonymName,drugSynonymId, sectionId, chapterNumber, Main.bookTitle + " : " + drugSynonymName );
							}
					...
					try {
							...
							while ((subTitleNode = subTitleNodeIt.nextNode()) != null) {
								...
								if(tempNode!=null){
									String currSubTitle = tempNode.getNodeValue();
									String tempSubTitle = currSubTitle.toLowerCase();
									int subTitleTopicIndex = StringUtil.findWordIndex(tempSubTitle, termToSearch);
									if(subTitleTopicIndex>-1){//sub-title topic found
										subTitleFound = true;
										addRISIndexToSect1(titleNode, termToTag, topicToTag, typeValue, termToSearch, topicToSearch, Main.XML_TAG_RULE_TYPE_ADDRISINDEX2);
										addDrugSynonymIndexDB(bookIsbn, drugSynonymName, drugSynonymId, sectionId, chapterNumber, Main.bookTitle + " : " + drugSynonymName );
			...
			}//end sect1 found drug term
			    }
			} catch (Exception e) {
				log.error("Unable to apply tagging rule for term = " + drugName);
				log.error(e.toString());
			}
		}//end drug synonym while
	}
	...
	public void addRISIndexToSect1(Node titleNode, String termToTag, String topicToTag, 
		...
		try {
			...
			log.info("Applied Rule Id = " + ruleId + " Drug Synonym = \"" + termToSearch + "\", Topic = \"" + topicToSearch + "\", Type = " + typeValue);
		} catch (Exception e) {
			log.error(e.toString());
		}
	}
	 ...
	 * @param displayTitle
	 */
	void addDrugSynonymIndexDB(String bookIsbn,String DrugSynonymName ,Integer drugSynonymId, String sectionId, String chapterNumber, String displayTitle){
		indexTopicDB.insertDrugSynonymResource(bookIsbn,DrugSynonymName , drugSynonymId, sectionId, chapterNumber, displayTitle);
	}
}
