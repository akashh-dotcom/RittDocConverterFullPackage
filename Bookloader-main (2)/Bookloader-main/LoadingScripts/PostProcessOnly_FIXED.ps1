...
function Resolve-ISBNList {
    ...
    # If it's an array, it may be:
    #  - multiple ISBNs, OR
    #  - a single element that is a file path, OR
    #  - a single element in the form "@path\to\isbnlist.txt"
    if ($InputValue -is [array]) {
        $items = @($InputValue | Where-Object { $_ -ne $null } | ForEach-Object { $_.ToString().Trim() } | Where-Object { $_ })
        ...
        if ($items.Count -eq 1) {
            ...
            if ([System.IO.File]::Exists($single)) {
                Write-LogLine "Reading ISBNs from file: $single" -Color Cyan
                $isbns = @(Get-Content $single |
    ...
    # Single value
    $singleValue = $InputValue.ToString().Trim()
    if ($singleValue.StartsWith('@')) {
        $singleValue = $singleValue.Substring(1).Trim()
    }

    if ([System.IO.File]::Exists($singleValue)) {
        Write-LogLine "Reading ISBNs from file: $singleValue" -Color Cyan
        $isbns = @(Get-Content $singleValue |
            ForEach-Object { $_.Trim() } |
            Where-Object { $_ -and -not $_.StartsWith('#') })
        Write-LogLine "Found $($isbns.Count) ISBN(s) in file" -Color Cyan
    }
